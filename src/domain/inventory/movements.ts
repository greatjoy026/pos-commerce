...
export interface CalculateMovementParams {
  ...
  movementId?: string;
  timestamp?: string;
  /** Legacy lifecycle metadata retained for domain-test/source compatibility; POS-001 does not mutate serials. */
  serialNumbers?: string[];
}
export interface MovementOutcome { updatedRecord: InventoryRecord; movementRecord: InventoryMovementRecord; }

/** Pure calculation only. Quantity is mandatory; missing quantity is a domain error, never an implicit unit sale. */
export function calculateMovementOutcome(record: InventoryRecord, params: CalculateMovementParams): MovementOutcome {
  const raw = params.quantityParam;
  if (typeof raw !== 'number' || !Number.isFinite(raw) || !Number.isInteger(raw) || raw === 0) throw new InventoryDomainError('quantityParam must be a non-zero finite integer', [{ field: 'quantityParam', message: 'quantityParam must be a non-zero finite integer', code: 'OUT_OF_RANGE' }]);
...
export function validateInventoryMovementRecord(input: unknown): InventoryMovementValidationResult {
  ...
  for (const field of ['id','inventoryId','sku','locationId','performedBy','timestamp']) if (typeof raw[field] !== 'string' || !(raw[field] as string).trim()) errors.push({ field, message: `${field} is required`, code: 'REQUIRED' });
