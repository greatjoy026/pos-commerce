...
} from '../../src/domain/inventory/types';
import { InventoryDomainError, validateInventoryRecord } from '../../src/domain/inventory/validation';
...
function assertNonZeroInteger(value: unknown): asserts value is number {
  if (typeof value !== 'number' || !Number.isFinite(value) || !Number.isInteger(value) || value === 0) {
...
function assertString(value: unknown, field: string, max = 128): asserts value is string {
  if (typeof value !== 'string' || value.trim().length === 0 || value.length > max) fail(`Invalid ${field}`);
}

function assertOptionalString(value: unknown, field: string, max = 128): void {
...
function assertOperationId(value: unknown): asserts value is string {
  ...
  if (!OPERATION_ID_PATTERN.test(value)) fail('Invalid operationId');
}

function assertAuthenticatedStaff(request: CallableRequest<unknown>): string {
  ...
  if (tenantId !== undefined && tenantId !== 'nexus-enterprise') {
    throw new HttpsError('permission-denied', 'Invalid enterprise scope');
  }
  const claimedRole = auth.token.role;
  if (auth.token.admin === true || auth.token.isSuperAdmin === true || (typeof claimedRole === 'string' && INVENTORY_ROLES.has(claimedRole))) {
    return auth.uid;
  }
  return verifyStaffRole(auth.uid);
}

function verifyStaffRole(uid: string): string {
  // This function is replaced by the async role check in the callable handlers.
  return uid;
}
...
async function getStaffActor(request: CallableRequest<unknown>): Promise<string> {
  ...
  if (typeof role !== 'string' || !INVENTORY_ROLES.has(role)) {
    throw new HttpsError('permission-denied', 'Inventory staff authorization required');
  }
  return auth.uid;
}

function movementId(operationId: string, suffix = ''): string {
...
function assertAvailable(record: InventoryRecord, quantity: number): void {
  ...
  if (quantity > available) throw new HttpsError('failed-precondition', `Only ${available} units are available`);
}
...
function buildMovement(
  ...
  if (!Number.isInteger(after) || after < 0) throw new HttpsError('failed-precondition', 'Movement would produce an invalid balance');
  if (movementType === 'ADJUSTMENT' && !reason?.trim()) throw new HttpsError('invalid-argument', 'An adjustment requires a reason');
  return {
    id,
...
function assertExisting(existing: InventoryMovementRecord, expected: InventoryMovementRecord): void {
  const same = existing.inventoryId === expected.inventoryId
  ...
  if (!same) throw new HttpsError('already-exists', 'operationId has already been used for a different operation');
}

async function executeMovement(request: CallableRequest<MovementRequest>, movementType: NonTransferMovement): Promise<InventoryMovementRecord> {
  ...
  if (movementType === 'ADJUSTMENT') assertNonZeroInteger(delta);
  else assertPositiveInteger(data.quantity, 'quantity');
  ...
  return db.runTransaction(async tx => {
    ...
    if (existingSnap.exists) {
      ...
      const expected = buildMovement(existing.id, inventory, movementType, delta, actor, data.referenceId, data.reason);
      assertExisting(existing, expected);
      return existing;
    }
    ...
    if (!validation.isValid) throw new HttpsError('failed-precondition', 'Movement violates inventory invariants');

    tx.update(inventoryRef, updated);
...
export const recordInventoryTransfer = onCall<TransferRequest>(async request => {
  ...
  return db.runTransaction(async tx => {
    ...
    if (outboundSnap.exists || inboundSnap.exists) {
      if (!outboundSnap.exists || !inboundSnap.exists) throw new HttpsError('already-exists', 'Transfer idempotency state is incomplete');
      const outbound = outboundSnap.data() as InventoryMovementRecord;
      const inbound = inboundSnap.data() as InventoryMovementRecord;
      const expectedOutbound = buildMovement(outbound.id, source, 'TRANSFER', -data.quantity, actor, data.referenceId, data.reason);
      const expectedInbound = buildMovement(inbound.id, destination, 'TRANSFER', data.quantity, actor, data.referenceId, data.reason);
      assertExisting(outbound, expectedOutbound);
      assertExisting(inbound, expectedInbound);
      return { outboundMovement: outbound, inboundMovement: inbound };
    }

    assertAvailable(source, data.quantity);
    const outbound = buildMovement(outboundRef.id, source, 'TRANSFER', -data.quantity, actor, data.referenceId, data.reason);
    const inbound = buildMovement(inboundRef.id, destination, 'TRANSFER', data.quantity, actor, data.referenceId, data.reason);
    ...
    if (!validateInventoryRecord(updatedSource).isValid || !validateInventoryRecord(updatedDestination).isValid) {
      throw new HttpsError('failed-precondition', 'Transfer violates inventory invariants');
    }
    tx.update(sourceRef, updatedSource);
    tx.update(destinationRef, updatedDestination);
