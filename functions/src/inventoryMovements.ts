...
function assertExisting(existing: InventoryMovementRecord, expected: InventoryMovementRecord): void {
  const same = existing.inventoryId === expected.inventoryId && existing.movementType === expected.movementType && existing.quantityDelta === expected.quantityDelta && existing.performedBy === expected.performedBy && existing.referenceId === expected.referenceId && existing.reason === expected.reason;
...
async function executeMovement(request: CallableRequest<MovementRequest>, movementType: NonTransferMovement): Promise<MovementResult> {
  ...
  return db.runTransaction(async tx => {
    ...
    tx.update(inventoryRef, updated); tx.create(movementRef, movement);
...
export const recordInventoryTransfer = onCall<TransferRequest>(async request => {
  ...
  const sourceRef = db.doc(`${INVENTORY}/${data.sourceInventoryId}`); const destinationRef = db.doc(`${INVENTORY}/${data.destinationInventoryId}`);
  const outboundRef = db.doc(`${MOVEMENTS}/${movementId(data.operationId, '_out')}`); const inboundRef = db.doc(`${MOVEMENTS}/${movementId(data.operationId, '_in')}`);
  return db.runTransaction(async tx => {
    ...
    if (outboundSnap.exists || inboundSnap.exists) {
      ...
      assertExisting(outbound, buildMovement(outbound.id, source, 'TRANSFER', -data.quantity, actor, data.referenceId, data.reason)); assertExisting(inbound, buildMovement(inbound.id, destination, 'TRANSFER', data.quantity, actor, data.referenceId, data.reason));
    ...
    tx.update(sourceRef, updatedSource); tx.update(destinationRef, updatedDestination); tx.create(outboundRef, outbound); tx.create(inboundRef, inbound);
