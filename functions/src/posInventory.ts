...
const db = getFirestore(
  adminApp,
  process.env.FIRESTORE_DATABASE_ID || 'ai-studio-nexusposcommerce-d2deaf29-88c9-4563-a26f-04f5e6504d77',
);
...
function fail(message: string, code: 'invalid-argument' | 'failed-precondition' = 'invalid-argument'): never {
...
function assertOrderId(value: unknown): asserts value is string {
  assertString(value, 'orderId', 128);
  if (!/^[A-Za-z0-9_-]+$/.test(value)) fail('Invalid orderId');
}
...
async function getStaffActor(request: CallableRequest<unknown>): Promise<string> {
  ...
  if (
    ...
    (typeof auth.token.role === 'string' && POS_ROLES.has(auth.token.role))
  ) {
    return auth.uid;
  }
  const staff = await db.doc(`staff/${auth.uid}`).get();
  const role = staff.exists ? staff.data()?.role : undefined;
  if (typeof role !== 'string' || !POS_ROLES.has(role)) {
...
/**
 * Authoritative POS sale boundary.
 *
 * The browser supplies catalog identity and selling quantity, never an inventory
 * document ID or balance. The server resolves the unique QUANTITY inventory
 * record for the requested SKU/location, validates the stored product/variant
 * binding, and atomically creates immutable SALE movements with the balance update.
 *
 * BATCH and SERIAL inventory are deliberately rejected until their dedicated
 * lifecycle/selection engines exist. NONE inventory is not a stock movement.
 */
export const recordPosSale = onCall<PosSaleRequest>(async (request) => {
  ...
  if (!Array.isArray(data.lines) || data.lines.length === 0 || data.lines.length > 100) {
    fail('POS sale must contain between 1 and 100 lines');
  }
  ...
  return db.runTransaction(async (tx) => {
    ...
    for (const line of lines) {
      ...
      if (inventorySnap.empty) {
        throw new HttpsError('not-found', `No inventory record exists for SKU ${line.sku} at ${line.locationId}`);
      }
      if (inventorySnap.size !== 1) {
        throw new HttpsError('failed-precondition', `Inventory resolution for SKU ${line.sku} is ambiguous`);
      }

      const inventorySnapDoc = inventorySnap.docs[0];
      const inventory = parseInventory(inventorySnapDoc.data(), inventorySnapDoc.id);

      if (inventory.productId !== line.productId) {
        throw new HttpsError('failed-precondition', `SKU ${line.sku} is not bound to the requested product`);
      }
      if ((inventory.variantId || undefined) !== (line.variantId || undefined)) {
        throw new HttpsError('failed-precondition', `SKU ${line.sku} is not bound to the requested variant`);
      }
      if (inventory.status !== 'ACTIVE') {
        throw new HttpsError('failed-precondition', `Inventory for SKU ${line.sku} is inactive`);
      }
      if (inventory.trackingMode === 'SERIAL' || inventory.trackingMode === 'BATCH') {
        throw new HttpsError('failed-precondition', `${inventory.trackingMode} inventory requires its dedicated POS lifecycle engine`);
      }
      if (inventory.trackingMode === 'NONE') {
        results.push({
          ...
          availableQuantityAfter: calculateAvailableQuantity(inventory),
          movementId: '',
        });
        continue;
      }
      ...
      if (line.quantity > available) {
        throw new HttpsError('failed-precondition', `Only ${available} units are available for SKU ${line.sku}`);
      }
      ...
      const expectedMovement: InventoryMovementRecord = {
        id: movementRef.id,
        inventoryId: inventory.id,
        sku: inventory.sku,
      ...
      if (existingSnap.exists) {
        ...
        if (!sameMovement(existing, expectedMovement)) {
          throw new HttpsError('already-exists', `operationId ${line.operationId} has already been used for another sale`);
        }
        const availableAfter = calculateAvailableQuantity({
          quantityOnHand: existing.quantityAfter,
      ...
      const updated: InventoryRecord = {
        ...inventory,
        quantityOnHand: expectedAfter,
        updatedAt: new Date().toISOString(),
      };
      if (!validateInventoryRecord(updated).isValid) {
        throw new HttpsError('failed-precondition', `Sale violates inventory invariants for SKU ${line.sku}`);
      }

      tx.update(inventorySnapDoc.ref, {
      ...
      results.push({
        ...
        availableQuantityAfter: calculateAvailableQuantity(updated),
