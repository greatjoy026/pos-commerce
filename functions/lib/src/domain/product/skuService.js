...
exports.resolveProductSku = resolveProductSku;
exports.extractAllProductSkus = extractAllProductSkus;
exports.generateCanonicalSku = generateCanonicalSku;
exports.validateSkuUniqueness = validateSkuUniqueness;
exports.validateBarcodeUniqueness = validateBarcodeUniqueness;
 ...
 */
function resolveProductSku(product, identifier) {
    if (!product || !identifier)
        return null;
    const query = identifier.trim().toLowerCase();
    const canonical = (0, projections_1.hasCanonicalProduct)(product)
        ? product.canonical
        : ((0, projections_1.isCanonicalProduct)(product) ? product : undefined);
    const raw = product;
    ...
    const variants = canonical?.variants || raw.variants || [];
    for (const v of variants) {
        ...
        if (vSku && vSku === query) {
            return {
                ...
                productId,
                variantId: v.id,
                variant: 'pricing' in v ? v : undefined
            };
        }
    ...
    const packagingUnits = canonical?.packagingUnits || raw.packagingUnits || raw.packaging?.packagingUnits || [];
 ...
 */
function extractAllProductSkus(product) {
    const results = [];
    const canonical = (0, projections_1.hasCanonicalProduct)(product)
        ? product.canonical
        : ((0, projections_1.isCanonicalProduct)(product) ? product : undefined);
 ...
 */
function generateCanonicalSku(basePrefix, attributes) {
 ...
 */
function validateSkuUniqueness(products, targetSku, excludeProductId) {
    if (!targetSku || typeof targetSku !== 'string')
        return false;
 ...
 */
function validateBarcodeUniqueness(products, targetBarcode, excludeProductId) {
    if (!targetBarcode || typeof targetBarcode !== 'string')
        return true; // Optional barcode
