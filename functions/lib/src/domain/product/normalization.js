...
exports.ProductNormalizationError = void 0;
exports.isLegacyProductInput = isLegacyProductInput;
exports.tryNormalizeProduct = tryNormalizeProduct;
exports.normalizeProduct = normalizeProduct;
exports.toLegacyProduct = toLegacyProduct;
exports.normalizeToLegacyProduct = normalizeToLegacyProduct;
/**
 * Structured Normalization Error
 */
class ProductNormalizationError extends Error {
    errors;
    constructor(errors) {
...
exports.ProductNormalizationError = ProductNormalizationError;
/**
 * Type guard for legacy or untrusted product input
 */
function isLegacyProductInput(data) {
 ...
 */
function tryNormalizeProduct(raw) {
    ...
    if (specifiedType) {
        if (VALID_PRODUCT_TYPES.includes(specifiedType)) {
            productType = specifiedType;
        }
        else {
            errors.push({
                field: 'productType',
    ...
    let status = 'Draft';
    if (specifiedStatus) {
        if (VALID_STATUSES.includes(specifiedStatus)) {
    ...
    if (Array.isArray(raw.packagingUnits)) {
        raw.packagingUnits.forEach((u, index) => {
        ...
        raw.packaging.sellingTiers.forEach((tier, index) => {
 ...
 */
function toLegacyProduct(canonical, legacyOperational) {
    ...
    const legacyVariants = canonical.variants.map((v) => {
    ...
    const legacyPackagingUnits = canonical.packagingUnits?.map(u => ({
        id: u.id,
        unitName: u.unitName,
        multiplier: u.multiplier,
 ...
 */
function normalizeToLegacyProduct(raw) {
