...
exports.isCanonicalProduct = isCanonicalProduct;
exports.hasCanonicalProduct = hasCanonicalProduct;
exports.computePublicAvailabilityStatus = computePublicAvailabilityStatus;
exports.toPublicCatalogProjection = toPublicCatalogProjection;
exports.toPOSProductView = toPOSProductView;
/**
 * Type guard for CanonicalProduct
 */
function isCanonicalProduct(p) {
 ...
 */
function hasCanonicalProduct(p) {
    return typeof p === 'object' && p !== null && 'canonical' in p && isCanonicalProduct(p.canonical);
}
/**
 * Default threshold for Low Stock classification.
 */
exports.DEFAULT_LOW_STOCK_THRESHOLD = 5;
/**
 ...
 */
function computePublicAvailabilityStatus(stock, lowStockThreshold = exports.DEFAULT_LOW_STOCK_THRESHOLD) {
    if (typeof stock !== 'number' || !Number.isFinite(stock) || stock <= 0) {
        return 'OUT_OF_STOCK';
    }
    const effectiveThreshold = typeof lowStockThreshold === 'number' &&
        Number.isFinite(lowStockThreshold) &&
        lowStockThreshold > 0
        ? lowStockThreshold
        : exports.DEFAULT_LOW_STOCK_THRESHOLD;
 ...
 */
function toPublicCatalogProjection(product, transitionalStockOverride) {
    const canonical = hasCanonicalProduct(product)
        ? product.canonical
    ...
    const variants = (canonical?.variants || raw.variants || []).map((v) => {
 ...
 */
function toPOSProductView(product, operationalFallback) {
    const canonical = hasCanonicalProduct(product)
        ? product.canonical
        : (isCanonicalProduct(product) ? product : undefined);
    const raw = product;
