...
exports.parseLegacyStock = parseLegacyStock;
exports.buildInventoryRecordId = buildInventoryRecordId;
exports.createInventoryRecord = createInventoryRecord;
exports.createInventoryRecordsFromLegacyProduct = createInventoryRecordsFromLegacyProduct;
exports.createInventoryRecordsFromCanonicalProduct = createInventoryRecordsFromCanonicalProduct;
exports.toLegacyProductWithInventory = toLegacyProductWithInventory;
const types_1 = require("./types");
const normalization_1 = require("../product/normalization");
 ...
 */
function parseLegacyStock(stock, context) {
    ...
    if (typeof stock !== 'number' || !Number.isFinite(stock) || !Number.isInteger(stock)) {
        throw new validation_1.InventoryDomainError(`Legacy migration failed for ${context}: stock must be a finite integer or omitted, received ${String(stock)}`, [{ field: `${context}.stock`, message: `Invalid legacy stock value: ${String(stock)}`, code: 'INVALID_TYPE' }]);
    }
    if (stock < 0) {
        throw new validation_1.InventoryDomainError(`Legacy migration failed for ${context}: stock cannot be negative, received ${stock}`, [{ field: `${context}.stock`, message: `Negative legacy stock value: ${stock}`, code: 'OUT_OF_RANGE' }]);
    }
    return stock;
}
/**
 ...
 */
function buildInventoryRecordId(sku, locationId, batchNumber) {
    const cleanSku = sku ? sku.toLowerCase().replace(/[^a-z0-9_-]/g, '_') : 'sku';
    const cleanLoc = locationId ? locationId.trim() : types_1.DEFAULT_LOCATION_ID;
...
function createInventoryRecord(params) {
    ...
    (0, validation_1.assertValidInventoryRecord)(record);
    return record;
}
/**
 ...
 */
function createInventoryRecordsFromLegacyProduct(product, defaultLocationId = types_1.DEFAULT_LOCATION_ID) {
    const records = [];
    const location = typeof product.location === 'string' && product.location.trim().length > 0
        ? product.location.trim()
        : defaultLocationId;
    const trackingMode = product.inventoryTracking === 'SERIAL'
        ? 'SERIAL'
 ...
 *   Inventory Record
 */
function createInventoryRecordsFromCanonicalProduct(canonical, defaultLocationId = types_1.DEFAULT_LOCATION_ID, initialStock = 0) {
    const records = [];
 ...
 */
function toLegacyProductWithInventory(canonical, inventoryRecords) {
    ...
    let primaryLocation = types_1.DEFAULT_LOCATION_ID;
    let primaryReorderPoint = 10;
    const legacyVariants = canonical.variants.map(variant => {
    ...
    const baseLegacy = (0, normalization_1.toLegacyProduct)(canonical);
    return {
        ...baseLegacy,
